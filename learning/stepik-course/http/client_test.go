...
func SlowHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(`{"users":[], "next_page":false}`))
}

func OriginNotAvailableHandler(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()

	file, err := os.Open("_dataset.xml")
	if err != nil {
...
func ErrorInvalidFormatHandler(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()

	w.WriteHeader(http.StatusBadRequest)
	w.Write([]byte(`{"some_random_thing":false}`))
}

func ErrorInvalidTemplateFormat(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()

	w.WriteHeader(http.StatusBadRequest)
	w.Write([]byte(`{"error": "where is json closing bracket?`))
}

func executeTestCases(t *testing.T, testCases []TestCase, handler func(req SearchRequest) (*SearchResponse, error)) {
	for testCaseInx, testCase := range testCases {
		tsResponse, err := handler(SearchRequest{
			Limit:      testCase.Input.Limit,
			Offset:     testCase.Input.Offset,
			Query:      testCase.Input.Query,
			OrderField: testCase.Input.OrderField,
			OrderBy:    testCase.Input.OrderBy,
		})

		if err != nil && !testCase.IsError {
			t.Errorf("Test %d - Error: %v", testCaseInx, err)
		}
		...
		if testCase.IsError && err != nil && !reflect.DeepEqual(testCase.ErrorMessage, err.Error()) {
			t.Errorf("Error results not equal:\nExpected: %+v\nGot: %+v", testCase.ErrorMessage, err)
		}

		if !reflect.DeepEqual(testCase.Result, tsResponse) {
			t.Errorf("Results not equal:\nExpected: %+v\nGot: %+v", testCase.Result, tsResponse)
		}
	}
}

func TestSearchServerLogic(t *testing.T) {
	testCases := []TestCase{
		{
			Input: &SearchRequest{
				Limit:      5,
		...
		},
		{
			...
			Result:       nil,
			ErrorMessage: "OrderBy -5 invalid",
			IsError:      true,
		},
		{
			Input: &SearchRequest{
			...
			Result:       nil,
			ErrorMessage: "OrderField Trash invalid",
			IsError:      true,
		},
		{
			Input: &SearchRequest{
		...
		},
		{
			...
			Result: &SearchResponse{
				Users: []User{{Id: 15, Name: "Allison Valdez", Age: 21, About: "Labore excepteur voluptate velit occaecat est nisi minim. Laborum ea et irure nostrud enim sit incididunt reprehenderit id est nostrud eu. Ullamco sint nisi voluptate cillum nostrud aliquip et minim. Enim duis esse do aute qui officia ipsum ut occaecat deserunt. Pariatur pariatur nisi do ad dolore reprehenderit et et enim esse dolor qui. Excepteur ullamco adipisicing qui adipisicing tempor minim aliquip.\n", Gender: "male"}, {Id: 16, Name: "Annie Osborn", Age: 35, About: "Consequat fugiat veniam commodo nisi nostrud culpa pariatur. Aliquip velit adipisicing dolor et nostrud. Eu nostrud officia velit eiusmod ullamco duis eiusmod ad non do quis.\n", Gender: "female"}, {Id: 19, Name: "Bell Bauer", Age: 26, About: "Nulla voluptate nostrud nostrud do ut tempor et quis non aliqua cillum in duis. Sit ipsum sit ut non proident exercitation. Quis consequat laboris deserunt adipisicing eiusmod non cillum magna.\n", Gender: "male"}, {Id: 22, Name: "Beth Wynn", Age: 31, About: "Proident non nisi dolore id non. Aliquip ex anim cupidatat dolore amet veniam tempor non adipisicing. Aliqua adipisicing eu esse quis reprehenderit est irure cillum duis dolor ex. Laborum do aute commodo amet. Fugiat aute in excepteur ut aliqua sint fugiat do nostrud voluptate duis do deserunt. Elit esse ipsum duis ipsum.\n", Gender: "female"}, {Id: 5, Name: "Beulah Stark", Age: 30, About: "Enim cillum eu cillum velit labore. In sint esse nulla occaecat voluptate pariatur aliqua aliqua non officia nulla aliqua. Fugiat nostrud irure officia minim cupidatat laborum ad incididunt dolore. Fugiat nostrud eiusmod ex ea nulla commodo. Reprehenderit sint qui anim non ad id adipisicing qui officia Lorem.\n", Gender: "female"}}, NextPage: true,
			},
			IsError: false,
		},
		{
...
func TestInvalidResponseJson(t *testing.T) {
	// dummy handler with some issues in logic
	...
	testCases := []TestCase{
		{
			...
			Result:       nil,
			ErrorMessage: "cant unpack result json: unexpected end of JSON input",
			IsError:      true,
		},
	}
	executeTestCases(
...
func TestTimeoutLimit(t *testing.T) {
	// slow handler with working time > 1s
	...
	testCases := []TestCase{
		{
			...
			Result:       nil,
			ErrorMessage: "timeout for limit=1&offset=0&order_by=0&order_field=&query=",
			IsError:      true,
		},
	}
	executeTestCases(
...
func TestInvalidServiceUrl(t *testing.T) {
	// unknown error of client (invaild client URL)
	client := SearchClient{
		AccessToken: "token",
		URL:         "www.sometrashnotwwserver.ll",
	}
	testCases := []TestCase{
		{
			Input: &SearchRequest{
			...
			Result:       nil,
			ErrorMessage: `unknown error Get "www.sometrashnotwwserver.ll?limit=1&offset=0&order_by=0&order_field=&query=Wolf": unsupported protocol scheme ""`,
			IsError:      true,
		},
	}
	executeTestCases(
...
func TestFailedConnectToOrigin(t *testing.T) {
	// not available data origin error
	ts := httptest.NewServer(http.HandlerFunc(OriginNotAvailableHandler))
	client := SearchClient{
		AccessToken: "token",
		URL:         ts.URL,
	}
...
func TestInvalidErrorFormat(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(ErrorInvalidFormatHandler))
	client := SearchClient{
		AccessToken: "token",
		URL:         ts.URL,
	}
...
func TestInvalidErrorData(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(ErrorInvalidTemplateFormat))
	client := SearchClient{
		AccessToken: "token",
		URL:         ts.URL,
	}
	testCases := []TestCase{
		{
			...
			Result:       nil,
			ErrorMessage: "cant unpack error json: unexpected end of JSON input",
			IsError:      true,
		},
	}
	executeTestCases(
